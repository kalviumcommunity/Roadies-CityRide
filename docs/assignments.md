# Roadies-CityRide Assignment Tracker

> **Repository:** [kalviumcommunity/Roadies-CityRide](https://github.com/kalviumcommunity/Roadies-CityRide)
> **Total Assignments:** 50
> **Last Updated:** 2026-08-13

## How to Use This Tracker

1. Pick an available assignment from the table below
2. Comment on the GitHub Issue to claim it
3. Create branch `assignment/XX-short-name`
4. Implement and push
5. Open PR with `Closes #ISSUE_NUMBER`
6. Update this tracker with your PR link

---

## Phase A — Product Definition (Issues #1–#5)

| # | Assignment | Issue | Status | Assignee | PR |
|---|---|---|---|---|---|
| 01 | Define the Ride Experience Analytics PRD | [#1](https://github.com/kalviumcommunity/Roadies-CityRide/issues/1) | Available | — | — |
| 02 | Define Measurable Business Questions | [#2](https://github.com/kalviumcommunity/Roadies-CityRide/issues/2) | Available | — | — |
| 03 | Define KPIs and Business Metrics | [#3](https://github.com/kalviumcommunity/Roadies-CityRide/issues/3) | Available | — | — |
| 04 | Define Analytical Scope and Methodology | [#4](https://github.com/kalviumcommunity/Roadies-CityRide/issues/4) | Available | — | — |
| 05 | Design Dashboard UX and User Stories | [#5](https://github.com/kalviumcommunity/Roadies-CityRide/issues/5) | Available | — | — |

## Phase B — Project/Repository Setup (Issues #6–#10)

| # | Assignment | Issue | Status | Assignee | PR |
|---|---|---|---|---|---|
| 06 | Design Repository Structure and Module Layout | [#6](https://github.com/kalviumcommunity/Roadies-CityRide/issues/6) | Completed | — | [#51](https://github.com/kalviumcommunity/Roadies-CityRide/pull/51) |
| 07 | Set Up Python Project with uv and pyproject.toml | [#7](https://github.com/kalviumcommunity/Roadies-CityRide/issues/7) | Completed | — | [#52](https://github.com/kalviumcommunity/Roadies-CityRide/pull/52) |
| 08 | Create Configuration and Environment Management | [#8](https://github.com/kalviumcommunity/Roadies-CityRide/issues/8) | Completed | — | [#53](https://github.com/kalviumcommunity/Roadies-CityRide/pull/53) |
| 09 | Set Up Git Workflow, Branching, and PR Conventions | [#9](https://github.com/kalviumcommunity/Roadies-CityRide/issues/9) | Available | — | — |
| 10 | Design Data Pipeline Structure and Entry Points | [#10](https://github.com/kalviumcommunity/Roadies-CityRide/issues/10) | Available | — | — |

## Phase C — Dataset Creation (Issues #11–#15)

| # | Assignment | Issue | Status | Assignee | PR |
|---|---|---|---|---|---|
| 11 | Design Synthetic Dataset Schema | [#11](https://github.com/kalviumcommunity/Roadies-CityRide/issues/11) | Completed | — | [#54](https://github.com/kalviumcommunity/Roadies-CityRide/pull/54) |
| 12 | Build Reproducible Synthetic Dataset Generator | [#12](https://github.com/kalviumcommunity/Roadies-CityRide/issues/12) | Available | — | — |
| 13 | Build CSV/JSON Data Ingestion Pipeline | [#13](https://github.com/kalviumcommunity/Roadies-CityRide/issues/13) | Available | — | — |
| 14 | Validate Dataset Sources and Cross-Checks | [#14](https://github.com/kalviumcommunity/Roadies-CityRide/issues/14) | Available | — | — |
| 15 | Create Data Dictionary and Business Context Mapping | [#15](https://github.com/kalviumcommunity/Roadies-CityRide/issues/15) | Available | — | — |

## Phase D — Data Quality (Issues #16–#24)

| # | Assignment | Issue | Status | Assignee | PR |
|---|---|---|---|---|---|
| 16 | Build Dataset Profiling and Quality Assessment | [#16](https://github.com/kalviumcommunity/Roadies-CityRide/issues/16) | Available | — | — |
| 17 | Implement Missing Value Detection and Imputation | [#17](https://github.com/kalviumcommunity/Roadies-CityRide/issues/17) | Available | — | — |
| 18 | Enforce Data Types and Standardisation | [#18](https://github.com/kalviumcommunity/Roadies-CityRide/issues/18) | Available | — | — |
| 19 | Implement Duplicate Detection and Record Deduplication | [#19](https://github.com/kalviumcommunity/Roadies-CityRide/issues/19) | Available | — | — |
| 20 | Build String Cleaning and Text Normalisation | [#20](https://github.com/kalviumcommunity/Roadies-CityRide/issues/20) | Available | — | — |
| 21 | Build Date and Time Transformation Pipeline | [#21](https://github.com/kalviumcommunity/Roadies-CityRide/issues/21) | Available | — | — |
| 22 | Implement Statistical Outlier Detection | [#22](https://github.com/kalviumcommunity/Roadies-CityRide/issues/22) | Available | — | — |
| 23 | Build Data Consistency and Validation Rules Engine | [#23](https://github.com/kalviumcommunity/Roadies-CityRide/issues/23) | Available | — | — |
| 24 | Build Multi-Source Merging and Join Validation | [#24](https://github.com/kalviumcommunity/Roadies-CityRide/issues/24) | Available | — | — |

## Phase E — Feature Engineering (Issues #25–#30)

| # | Assignment | Issue | Status | Assignee | PR |
|---|---|---|---|---|---|
| 25 | Engineer Demand and Supply Features | [#25](https://github.com/kalviumcommunity/Roadies-CityRide/issues/25) | Available | — | — |
| 26 | Engineer Surge Pricing Features | [#26](https://github.com/kalviumcommunity/Roadies-CityRide/issues/26) | Available | — | — |
| 27 | Engineer Driver Acceptance Features | [#27](https://github.com/kalviumcommunity/Roadies-CityRide/issues/27) | Available | — | — |
| 28 | Engineer Rider Cancellation Features | [#28](https://github.com/kalviumcommunity/Roadies-CityRide/issues/28) | Available | — | — |
| 29 | Engineer Rider Experience Score Features | [#29](https://github.com/kalviumcommunity/Roadies-CityRide/issues/29) | Available | — | — |
| 30 | Define High-Demand Period Classification | [#30](https://github.com/kalviumcommunity/Roadies-CityRide/issues/30) | Available | — | — |

## Phase F — Python Analysis (Issues #31–#40)

| # | Assignment | Issue | Status | Assignee | PR |
|---|---|---|---|---|---|
| 31 | Analyse Feature Distributions for Business Trends | [#31](https://github.com/kalviumcommunity/Roadies-CityRide/issues/31) | Available | — | — |
| 32 | Perform Correlation and Relationship Analysis | [#32](https://github.com/kalviumcommunity/Roadies-CityRide/issues/32) | Available | — | — |
| 33 | Build City Segmentation and Comparison Analysis | [#33](https://github.com/kalviumcommunity/Roadies-CityRide/issues/33) | Available | — | — |
| 34 | Perform GroupBy Aggregation and Segment Insights | [#34](https://github.com/kalviumcommunity/Roadies-CityRide/issues/34) | Available | — | — |
| 35 | Perform Time-Series Trend and Rolling Metrics Analysis | [#35](https://github.com/kalviumcommunity/Roadies-CityRide/issues/35) | Available | — | — |
| 36 | Perform Behavioural Analysis and User Segmentation | [#36](https://github.com/kalviumcommunity/Roadies-CityRide/issues/36) | Available | — | — |
| 37 | Perform Funnel Analysis and Drop-Off Detection | [#37](https://github.com/kalviumcommunity/Roadies-CityRide/issues/37) | Available | — | — |
| 38 | Perform Root-Cause Investigation of Degraded Cities | [#38](https://github.com/kalviumcommunity/Roadies-CityRide/issues/38) | Available | — | — |
| 39 | Perform NumPy Vectorised Computation Workflow | [#39](https://github.com/kalviumcommunity/Roadies-CityRide/issues/39) | Available | — | — |
| 40 | Detect Anomalies and Operational Risk Patterns | [#40](https://github.com/kalviumcommunity/Roadies-CityRide/issues/40) | Available | — | — |

## Phase G — SQL Analytics (Issues #41–#45)

| # | Assignment | Issue | Status | Assignee | PR |
|---|---|---|---|---|---|
| 41 | Set Up SQL Database Integration | [#41](https://github.com/kalviumcommunity/Roadies-CityRide/issues/41) | Available | — | — |
| 42 | Design SQL Business Metrics Queries | [#42](https://github.com/kalviumcommunity/Roadies-CityRide/issues/42) | Available | — | — |
| 43 | Build SQL Joins, Window Functions, and Advanced Analysis | [#43](https://github.com/kalviumcommunity/Roadies-CityRide/issues/43) | Available | — | — |
| 44 | Optimise SQL Queries and Build Aggregation Views | [#44](https://github.com/kalviumcommunity/Roadies-CityRide/issues/44) | Available | — | — |
| 45 | Validate SQL Results Against Python Analysis | [#45](https://github.com/kalviumcommunity/Roadies-CityRide/issues/45) | Available | — | — |

## Phase H — Visual Product (Issues #46–#48)

| # | Assignment | Issue | Status | Assignee | PR |
|---|---|---|---|---|---|
| 46 | Apply Business Visualisation Principles and Plotly Charts | [#46](https://github.com/kalviumcommunity/Roadies-CityRide/issues/46) | Available | — | — |
| 47 | Design KPI Cards and Summary Metrics | [#47](https://github.com/kalviumcommunity/Roadies-CityRide/issues/47) | Available | — | — |
| 48 | Build Streamlit Dashboard with Filters and Navigation | [#48](https://github.com/kalviumcommunity/Roadies-CityRide/issues/48) | Available | — | — |

## Phase I — Productionization (Issues #49–#50)

| # | Assignment | Issue | Status | Assignee | PR |
|---|---|---|---|---|---|
| 49 | Build Alert Monitoring and Automated Pipeline Execution | [#49](https://github.com/kalviumcommunity/Roadies-CityRide/issues/49) | Available | — | — |
| 50 | Deliver Final Integrated Data Product with Documentation | [#50](https://github.com/kalviumcommunity/Roadies-CityRide/issues/50) | Available | — | — |

---

## Status Legend

| Status | Meaning |
|---|---|
| **Available** | Ready to be picked up |
| **In Progress** | Someone is working on it |
| **Completed** | PR merged |

## Workflow

1. Comment on the issue to claim it
2. Create branch: `assignment/XX-short-name`
3. Implement the assignment
4. Push and open a PR
5. In the PR description, write: `Closes #ISSUE_NUMBER`
6. Update this tracker: set Status to "In Progress" with your PR link
7. After merge: Status becomes "Completed"
